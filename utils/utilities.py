...
class Utilities:
    ...
    def getFileTuple(self, path):
        return os.path.split(path)

    def getFileName(self, path):
        return self.getFileTuple(path)[1]

    def calculate_adjusted_review_count(self, data):
        adjusted_count = 0
        adjusted_rating = 0.0
    ...
    def search(self, data, key, value):
        for d in data:
            if d[key] != None and d[key] == value:
                return d
        return None

    def search_fuzzy(self, value, data):
        try:
    ...
    def merge_dir(self, inputPath, summaryPath):
        input_list = self.listFiles(inputPath)
        with tempfile.TemporaryDirectory() as tempDir:
            for file in input_list:
                file_name = self.getFileName(file)[1]
                with open(file, "r") as inputFile:
                    ...
                    with open(summary_file_path , "r") as summaryFile:
                        ...
                        summaryFile.close()
                    inputFile.close()
            tmp_files = self.listFiles(tempDir)
            for tmp_file in tmp_files:
                tmp_file_name = self.getFileName(tmp_file)[1]
                shutil.copy(tmp_file, summaryPath + tmp_file_name)
            shutil.rmtree(tempDir)

    def merge_file(self, inputPath, summaryPath):
        with tempfile.TemporaryDirectory() as tempDir:
            file_name = self.getFileName(inputPath)[1]
