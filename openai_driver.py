...
def get_prompt(file_content):
     return f'''Create an article for the {file_content["company_type"]} {file_content["name"]} with the following requirements: 
              ...
              7. When referring to the subject of the article, if the value associated with the key "company_type" is "property" refer to it as a property. If the value is "company", refer to it as a company.
        ...
        for file in dir_list:
          ...
          with open(f"{path}/{file}", "r") as input_file:
              ...
              with open(f'{out}/{file_json["slug"]}.json', 'w') as output_file:
                  json.dump({**file_json, "summary": result.choices[0].message.content}, output_file,  ensure_ascii=True, indent=2)
                  output_file.close()
...
async def async_driver(path, out):
  ...
  tasks = [rate_limiter(f"{path}{file}", semaphore) for file in file_list[0:3]]
  result = await gather(*tasks)
  for res in result:   
      print(json.dumps(res, indent=2))
          
# run(async_driver("./merged_reviews/", "./summaries/"))
